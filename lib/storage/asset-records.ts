...
import { contactSubmissions, customerArtworks, designs, heroSlides, offers, orders, productCategories, productImages, storageAssets, templates } from '@/lib/db/schema'
...
export async function getAssetReferences(assetId: string, key: string) {
  const [images, templateRows, heroes, offerRows, artworks, designRows, categories, orderDocuments, supportRequests] = await Promise.all([
    ...
    db.select({ id: contactSubmissions.id }).from(contactSubmissions).where(sql`${contactSubmissions.supportPayload}->'artwork'->>'key' = ${key}`),
  ])
  return { products: images.length, templates: templateRows.length, heroes: heroes.length, offers: offerRows.length, artworks: artworks.length, designs: designRows.length, categories: categories.length, orderDocuments: orderDocuments.length, supportRequests: supportRequests.length, total: supportRequests.length + images.length + templateRows.length + heroes.length + offerRows.length + artworks.length + designRows.length + categories.length + orderDocuments.length }
}
