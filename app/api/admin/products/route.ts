...
export async function GET() {
  ...
  try {
    const [productRows, categories, templateRows] = await Promise.all([
      ...
      db.select({ id: templates.id, name: templates.name, status: templates.status, conversionStatus: templates.conversionStatus, templateSide: templates.templateSide }).from(templates).orderBy(asc(templates.name)),
    ])
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const referencedTemplateIds = [...new Set(input.sizes.flatMap((size) => size.designConfigurations.flatMap((configuration) => [configuration.singleTemplateId, configuration.frontTemplateId, configuration.backTemplateId].filter((id): id is string => Boolean(id)))))]
    ...
    const message = productWriteErrorMessage(error, 'created')
