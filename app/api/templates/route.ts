...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rows, productRows, sizes, categories] = await Promise.all([
      db.select({ id: templates.id, name: templates.name, description: templates.description, previewImageUrl: templates.previewImageUrl, previewImageKey: templates.previewImageKey, templateVersion: templates.templateVersion, templateSide: templates.templateSide, logicalCanvasWidth: templates.logicalCanvasWidth, logicalCanvasHeight: templates.logicalCanvasHeight, conversionStatus: templates.conversionStatus, status: templates.status, updatedAt: templates.updatedAt }).from(templates).where(inArray(templates.id, templateIds)).orderBy(asc(templates.name)),
