...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ data: {
      template: { id: template.id, name: template.name, version: template.templateVersion, templateKind: template.templateKind, templateSide: template.templateSide, printableArea: template.printableArea, maskReference: template.fixedSvgKey, ...(editorData ? { canvasData: template.canvasData, fixedCanvasData: template.fixedCanvasData, baseCanvasWidth: template.logicalCanvasWidth, baseCanvasHeight: template.logicalCanvasHeight } : { previewUrl: canonicalStoredAssetUrl(template.previewImageUrl, template.previewImageKey), description: template.description, category: category?.name || 'Products', subcategory: product.name }) },
      backTemplate: backTemplate ? { id: backTemplate.id, name: backTemplate.name, version: backTemplate.templateVersion, templateKind: backTemplate.templateKind, templateSide: backTemplate.templateSide, printableArea: backTemplate.printableArea, maskReference: backTemplate.fixedSvgKey, ...(editorData ? { canvasData: backTemplate.canvasData, fixedCanvasData: backTemplate.fixedCanvasData, baseCanvasWidth: backTemplate.logicalCanvasWidth, baseCanvasHeight: backTemplate.logicalCanvasHeight } : { previewUrl: canonicalStoredAssetUrl(backTemplate.previewImageUrl, backTemplate.previewImageKey) }) } : null,
