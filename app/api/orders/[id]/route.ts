...
export async function GET(_: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const items = await db.select().from(orderItems).where(eq(orderItems.orderId, id)); const productIds = items.map((item) => item.productId).filter((id): id is string => Boolean(id)); const artworkIds = items.flatMap((item) => item.customerArtworkId ? [item.customerArtworkId] : []); const designIds = items.flatMap((item) => item.designId ? [item.designId] : [])
  ...
  return NextResponse.json({ data: { order: { ...order, internalNotes: undefined, deadline: deadlineState(order.designConfirmationDeadline, order.designConfirmedAt), items: items.map((item) => ({ ...item, product: productRows.find((product) => product.id === item.productId) || (item.productId === null && item.customerArtworkId ? { id: CUSTOM_PRODUCT_ID, name: CUSTOM_PRODUCT_NAME, sku: CUSTOM_PRODUCT_SKU } : null), artworkUrl: item.customerArtworkId ? urls.get(item.customerArtworkId) : null, designUrl: item.designId ? designUrls.get(item.designId) : null })), history: history.map(({ internalNote: _, ...row }) => row), payments } } }, { headers: { 'cache-control': 'private, no-store' } })
}
