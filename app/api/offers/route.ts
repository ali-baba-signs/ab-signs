...
export async function GET() {
  ...
  return NextResponse.json({ data: { offers: visible.map((row) => ({ ...row, offer: { ...row.offer, imageUrl: assets.find((asset) => asset.id === row.offer.imageAssetId) ? getStoredAssetUrl(assets.find((asset) => asset.id === row.offer.imageAssetId)!.objectKey) : canonicalStoredAssetUrl(row.offer.imageUrl), mobileImageUrl: assets.find((asset) => asset.id === row.offer.mobileImageAssetId) ? getStoredAssetUrl(assets.find((asset) => asset.id === row.offer.mobileImageAssetId)!.objectKey) : canonicalStoredAssetUrl(row.offer.mobileImageUrl) } })) } })
}
