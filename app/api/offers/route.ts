...
export async function GET() {
  ...
  return NextResponse.json({ data: { offers: rows.filter(({ coupon }) => !coupon || (coupon.visibility === 'public' && coupon.active && (!coupon.startsAt || coupon.startsAt <= now) && (!coupon.endsAt || coupon.endsAt >= now) && (coupon.usageLimit === null || coupon.usedCount + coupon.reservedCount < coupon.usageLimit))) } })
}
