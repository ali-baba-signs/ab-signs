...
import { orderItems, orders, productReviews, users } from '@/lib/db/schema'
...
export async function GET(request: NextRequest) {
  ...
  const rows = await db.select({ review: productReviews, customerName: users.name }).from(productReviews).innerJoin(users, eq(productReviews.userId, users.id)).where(mine ? eq(productReviews.userId, session!.user.id) : and(eq(productReviews.productId, productId), eq(productReviews.moderationStatus, 'published'))).orderBy(desc(productReviews.createdAt))
  const reviews = rows.map(({ review, customerName }) => { const parts = customerName.trim().split(/\s+/); const displayName = parts.length > 1 ? `${parts[0]} ${parts.at(-1)?.[0]}.` : parts[0]; return { ...review, displayName } })
