...
export async function GET(_: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const compatibleTemplates = product.templates.filter((template) => template.status === 'active' && template.conversionStatus === 'ready' && template.compatibleSizeIds.length > 0)
