...
import { DESIGN_UPLOAD_ERROR, sanitizeFilename } from '@/lib/storage/upload-validation'
import { sanitizeSvgMarkup } from '@/lib/templates/svg-sanitization'
import { createHash } from 'node:crypto'

const allowed = new Map([
  ['application/pdf', ['pdf', 'ai']], ['image/png', ['png']], ['image/svg+xml', ['svg']],
  ['application/postscript', ['eps', 'ai']], ['application/eps', ['eps']], ['application/x-eps', ['eps']],
  ['application/illustrator', ['ai']], ['application/vnd.adobe.illustrator', ['ai']],
])
const MAX_BYTES = 100 * 1024 * 1024
function safeText(value: FormDataEntryValue | null, max = 2000) { return typeof value === 'string' ? value.trim().slice(0, max) : '' }
function validateBytes(extension: string, type: string, body: Buffer) { const pdf=body.subarray(0,5).equals(Buffer.from('%PDF-'));const postscript=body.subarray(0,20).toString('ascii').startsWith('%!PS-Adobe');if(extension==='pdf'&&!pdf)throw new Error('The PDF signature is invalid.');if(extension==='png'&&body.subarray(0,8).toString('hex')!=='89504e470d0a1a0a')throw new Error('The PNG signature is invalid.');if(extension==='eps'&&!postscript)throw new Error('The EPS signature is invalid.');if(extension==='ai'&&!pdf&&!postscript)throw new Error('The AI file signature is invalid.');if(!allowed.get(type)?.includes(extension))throw new Error(DESIGN_UPLOAD_ERROR) }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const extension = file.name.split('.').pop()?.toLowerCase() || ''; const suppliedType=file.type||(extension==='eps'?'application/postscript':extension==='ai'?'application/vnd.adobe.illustrator':''); if (!allowed.get(suppliedType)?.includes(extension)) throw new Error(DESIGN_UPLOAD_ERROR)
    ...
    const source = Buffer.from(await file.arrayBuffer()); validateBytes(extension, suppliedType, source)
    if (suppliedType === 'image/svg+xml') sanitizeSvgMarkup(source.toString('utf8'))
    const body = source
    const contentType=extension==='ai'?'application/vnd.adobe.illustrator':extension==='eps'?'application/postscript':suppliedType
    const dimensions = contentType === 'image/png' ? pngDimensions(source) : contentType === 'image/svg+xml' ? svgDimensions(source.toString('utf8')) : null
    key = `${R2_PATHS.artworkUploads}/${session.user.id.replace(/[^a-zA-Z0-9_-]/g, '')}/${crypto.randomUUID()}-${sanitizeFilename(file.name)}`
    const checksum = createHash('sha256').update(body).digest('hex')
    await uploadObject({ key, body, contentType, metadata: { owner: session.user.id, originalName: file.name.slice(0, 250), checksum, private: 'true' } })
    const artwork = await db.transaction(async (tx) => {
      const asset = await registerStorageAsset({ key, contentType, size: body.length, uploadedBy: null, etag: checksum })
    ...
    return NextResponse.json({ data: { artwork: { ...artwork, contentType, fileSize: body.length, previewUrl: ['image/png','image/svg+xml','application/pdf'].includes(contentType) ? `/api/artwork/preview?id=${artwork.id}` : null } } }, { status: 201 })
