...
import { paymentEventForIntent, reconcileStripePayment } from '@/lib/payments/reconcile'

export async function POST(request: NextRequest) {
  try {
    const { orderId, checkoutToken } = await request.json() as { orderId?: string; checkoutToken?: string }
    if (!orderId || !checkoutToken) return NextResponse.json({ error: { message: 'Order authorization is required.' } }, { status: 400 })
    ...
    if (!['paid', 'cancelled'].includes(data.order.paymentStatus) && process.env.STRIPE_SECRET_KEY) {
      try {
        const [payment] = await db.select().from(paymentRecords).where(and(eq(paymentRecords.orderId, data.order.id), eq(paymentRecords.provider, 'stripe'))).limit(1)
        if (payment?.externalId) {
          ...
          await reconcileStripePayment({ intent, eventType, eventId: `reconcile:${intent.id}:${intent.status}:${intent.last_payment_error?.code || 'none'}` })
          const refreshed = await loadPaymentOrder(orderId)
          if (refreshed) return NextResponse.json({ data: paymentSummary(refreshed) }, { headers: { 'cache-control': 'private, no-store' } })
        }
      } catch (error) {
        console.error('Stripe status reconciliation failed; returning the safely stored order status.', error)
      }
    }
    ...
    return NextResponse.json({ error: { message: 'Payment status is temporarily unavailable.' } }, { status: 400 })
  }
}
