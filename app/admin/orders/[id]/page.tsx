...
interface OrderDetail {
  ...
  items: Array<{ id: string; quantity: number; designSource: string; totalPrice: string; specifications: Record<string, string> | null; product?: { name: string }; artwork?: { originalFilename: string; contentType: string; fileSize: number; notes: string | null; sourceWidthPx: number | null; sourceHeightPx: number | null; originalUrl: string; previewUrl: string | null; dimensionWarning: string | null }; designUrl?: string; productionPreviewUrl?: string; productionUrl?: string; backProductionPreviewUrl?: string; backProductionUrl?: string }>
  ...
  return <main className="min-h-screen bg-background px-4 py-8"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_380px]">
      <div className="space-y-5">
        <section className="rounded-xl border bg-card p-5"><h2 className="font-bold">Items and printable artwork</h2>{order.items.map((item) => { const specs=item.specifications||{}; return <article key={item.id} className="mt-4 rounded-lg border p-4"><div className="flex flex-wrap justify-between gap-2"><div><p className="font-semibold">{specs.productName || item.product?.name || 'Historical product'} × {item.quantity}</p><p className="text-sm text-muted-foreground">{specs.variant || specs.sizeLabel || 'Standard'} · {specs.height && specs.width ? `${specs.height} × ${specs.width} ${specs.unit}` : 'Dimensions unavailable'} · {specs.sideMode === 'double' ? 'Double-sided' : 'Single-sided'}</p></div><span className="h-fit rounded-full bg-secondary px-3 py-1 text-xs font-bold">{item.designSource.replaceAll('_',' ')}</span></div>{item.designSource === 'online_editor' && <div className="mt-4 grid gap-4 md:grid-cols-2"><div><h3 className="text-sm font-bold">Front production artwork</h3>{item.productionPreviewUrl && <img src={item.productionPreviewUrl} alt="Front production artwork preview" className="mt-2 max-h-72 w-full rounded border object-contain" />}<div className="mt-2 flex flex-wrap gap-3">{item.productionUrl && <a className="inline-flex items-center gap-1 font-semibold text-primary" href={item.productionUrl}><Download/> Production PDF</a>}{item.designUrl && <a className="inline-flex items-center gap-1 text-sm text-primary" href={item.designUrl}><Download/> Saved design JSON</a>}</div></div>{specs.sideMode === 'double' && <div><h3 className="text-sm font-bold">Back production artwork</h3>{item.backProductionPreviewUrl && <img src={item.backProductionPreviewUrl} alt="Back production artwork preview" className="mt-2 max-h-72 w-full rounded border object-contain" />}{item.backProductionUrl && <a className="mt-2 inline-flex items-center gap-1 font-semibold text-primary" href={item.backProductionUrl}><Download/> Back production PDF</a>}</div>}</div>}{item.designSource === 'customer_upload' && item.artwork && <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_260px]"><div><p className="font-semibold">Original customer file: {item.artwork.originalFilename}</p><p className="text-sm text-muted-foreground">{item.artwork.contentType} · {(item.artwork.fileSize/1024/1024).toFixed(2)} MB{item.artwork.sourceWidthPx ? ` · ${item.artwork.sourceWidthPx} × ${item.artwork.sourceHeightPx} px` : ''}</p>{item.artwork.notes && <p className="mt-2 text-sm">Customer notes: {item.artwork.notes}</p>}<p className="mt-2 text-xs text-amber-700">Original authorized file — no bleed, resizing, cropping, or cut marks have been applied.</p><a className="mt-3 inline-flex items-center gap-1 font-semibold text-primary" href={item.artwork.originalUrl}><Download/> Download original production file</a></div>{item.artwork.previewUrl ? item.artwork.contentType === 'application/pdf' ? <object data={`${item.artwork.previewUrl}#page=1&view=FitH`} type="application/pdf" className="h-64 w-full rounded border" aria-label="Customer PDF preview"/> : <img src={item.artwork.previewUrl} alt="Customer artwork preview" className="max-h-64 w-full rounded border object-contain"/> : <div className="grid h-40 place-items-center rounded border border-dashed text-center text-sm">EPS preview unavailable<br/>Metadata shown only</div>}</div>}<p className="mt-3 text-sm">Artwork status: <b>Ready for production review</b></p></article>})}</section>
