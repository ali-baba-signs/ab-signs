...
import { ORDER_MILESTONE_LABELS, orderMilestone, orderMilestoneLabel } from '@/lib/orders/workflow'

interface OrderItem { id: string; specifications?: Record<string, string> | null; product?: { id: string; name: string; sku: string; categoryId: string; categoryName: string } | null }
...
export default function OrdersManagementPage() {
  ...
  const visible = useMemo(() => orders.filter((order) => {
    ...
    const text = `${order.orderNumber} ${order.customerEmail} ${order.items.map((item) => `${item.product?.name || ''} ${item.product?.sku || item.specifications?.sku || ''} ${item.product?.categoryName || ''}`).join(' ')}`.toLowerCase()
