...
interface TemplateRow {
  ...
  fixedSvgUrl: string | null; fixedSvgKey: string | null; fixedSvgAssetId: string | null; templateKind: 'banner' | 'flag'; templateSide: 'single' | 'front' | 'back'; printableArea: { x: number; y: number; width: number; height: number } | null
...
const emptyForm = { name: '', description: '', categoryId: '', productIds: [] as string[], status: 'draft', templateKind: 'flag', templateSide: 'single' as 'single' | 'front' | 'back', width: '', height: '', unit: 'mm' as MeasurementUnit }
...
export default function TemplatesPage() {
  ...
  function start(row?: TemplateRow) {
    ...
    setForm({ name: row?.name || '', description: row?.description || '', categoryId, productIds: productIds.length ? productIds : product ? [product.id] : [], status: row?.status || 'draft', templateKind: row?.templateKind || 'banner', templateSide: row?.templateSide || 'single', ...dimensionsFor(product) })
  ...
  return <main className="min-h-screen bg-background px-4 py-8"><div className="mx-auto max-w-6xl">
    ...
    <form onSubmit={save} className="mt-6 rounded-xl border bg-card p-6"><h2 className="text-xl font-bold">{editing ? `Edit ${editing.name}` : 'Create from SVG'}</h2>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        ...
        {definitions.filter(({ field }) => field !== 'previewImage' && (field !== 'fixedSvg' || form.templateKind === 'flag')).map(({ field, label, accept, icon: Icon }) => { const asset = assets[field]; return <div key={field} className="rounded-lg border p-4"><div className="flex items-center gap-2 font-semibold"><Icon className="h-4 w-4 text-primary" /> {label}</div><p className="mt-2 text-xs text-muted-foreground">{field === 'fixedSvg' ? 'Non-editable product silhouette, print boundary, and clipping mask.' : 'Only artwork from this file appears in the editable layer list.'}</p><p className="mt-3 truncate text-xs">{asset?.filename || asset?.key || 'No file selected.'}</p><label className="mt-3 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-md border-2 border-primary bg-primary px-4 py-2 text-sm font-bold text-primary-foreground shadow-sm"><Upload className="h-4 w-4" /> {asset ? 'Replace File' : 'Choose File'}<input type="file" accept={accept} className="sr-only" onChange={(event) => void upload(field, event.target.files?.[0])} /></label>{progress[field] !== undefined && progress[field]! < 100 && <p className="mt-2 text-xs">Uploading {progress[field]}%</p>}</div> })}
      </div>
