...
const emptyForm = { name: '', description: '', categoryId: '', productIds: [] as string[], status: 'draft', width: '', height: '', unit: 'mm' as MeasurementUnit }
...
export default function TemplatesPage() {
  ...
  function start(row?: TemplateRow) {
    ...
    setForm({ name: row?.name || '', description: row?.description || '', categoryId, productIds: productIds.length ? productIds : product ? [product.id] : [], status: row?.status || 'draft', ...dimensionsFor(product) })
  ...
  async function sourceForGeneration() {
    ...
    const response = await fetch(`/api/admin/templates/source?assetId=${encodeURIComponent(assets.svg.id)}`, { cache: 'no-store' })
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch(editing ? `/api/admin/templates/${editing.id}` : '/api/admin/templates', {
        ...
        body: JSON.stringify({ ...form, categoryId: selectedProduct?.categoryId || form.categoryId, assets, svgChecksum: assets.svg.checksum || editing?.svgChecksum, conversionVersion: 1, regenerate: shouldGenerate, canvasData: generated?.canvasData, scaleMetadata: generated ? { widthMm: generated.widthMm, heightMm: generated.heightMm, pixelsPerMm: generated.pixelsPerMm, sourceObjectCount: generated.sourceObjectCount, sourceToCanvasScale: generated.scale } : null }),
      })
      const payload = await response.json()
  ...
  return <main className="min-h-screen bg-background px-4 py-8"><div className="mx-auto max-w-6xl">
    ...
    <form onSubmit={save} className="mt-6 rounded-xl border bg-card p-6"><h2 className="text-xl font-bold">{editing ? `Edit ${editing.name}` : 'Create from SVG'}</h2>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        ...
        {definitions.filter(({ field }) => field === 'svg').map(({ field, label, accept, icon: Icon }) => { const asset = assets[field]; return <div key={field} className="rounded-lg border p-4"><div className="flex items-center gap-2 font-semibold"><Icon className="h-4 w-4 text-primary" /> {label}</div><p className="mt-3 truncate text-xs">{asset?.filename || asset?.key || 'No file selected.'}</p><label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded border px-3 py-2 text-xs font-semibold"><Upload className="h-3 w-3" /> {asset ? 'Replace' : 'Choose file'}<input type="file" accept={accept} className="sr-only" onChange={(event) => void upload(field, event.target.files?.[0])} /></label>{progress[field] !== undefined && progress[field]! < 100 && <p className="mt-2 text-xs">Uploading {progress[field]}%</p>}</div> })}
      </div>
