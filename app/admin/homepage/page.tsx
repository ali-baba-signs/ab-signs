...
import { ArrowDown, ArrowLeft, ArrowUp, Edit2, ImagePlus, Plus, Save, Trash2 } from 'lucide-react'
...
export default function HomepageAdminPage() {
  ...
  return <main className="min-h-screen bg-zinc-50 px-4 py-8"><div className="mx-auto max-w-7xl"><Link href={adminPath()} className="inline-flex items-center gap-2 text-sm font-semibold"><ArrowLeft className="h-4 w-4" /> Dashboard</Link><div className="mt-2"><h1 className="text-3xl font-black">Homepage hero carousel</h1><p className="mt-2 text-sm text-zinc-500">Only enabled and featured slides appear publicly, ordered from lowest to highest.</p></div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[420px_1fr]"><form onSubmit={save} className="h-fit rounded-xl border bg-white p-5"><h2 className="text-xl font-bold">{editingId ? 'Edit hero slide' : 'Create hero slide'}</h2><div className="mt-4 space-y-4">
      <div><p className="text-sm font-semibold">Desktop image</p>{desktopPreview && <img src={desktopPreview} alt="Desktop preview" className="mt-2 h-32 w-full rounded object-cover" />}<label className="mt-2 inline-flex cursor-pointer items-center gap-2 rounded border px-3 py-2 text-xs font-semibold"><ImagePlus className="h-4 w-4" /> {form.desktopAssetId ? 'Replace desktop image' : 'Upload desktop image'}<input required={!form.desktopAssetId} type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => void upload('desktop', event.target.files?.[0])} /></label></div>
      <div><p className="text-sm font-semibold">Mobile image (optional)</p>{mobilePreview && <img src={mobilePreview} alt="Mobile preview" className="mt-2 h-36 w-28 rounded object-cover" />}<label className="mt-2 inline-flex cursor-pointer items-center gap-2 rounded border px-3 py-2 text-xs font-semibold"><ImagePlus className="h-4 w-4" /> {form.mobileAssetId ? 'Replace mobile image' : 'Upload mobile image'}<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => void upload('mobile', event.target.files?.[0])} /></label></div>
