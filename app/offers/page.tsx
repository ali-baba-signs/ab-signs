...
export default async function OffersPage() {
  ...
  const visible = rows.filter(({ coupon }) => !coupon || (coupon.visibility === 'public' && coupon.active && (!coupon.endsAt || coupon.endsAt >= now) && (coupon.usageLimit === null || coupon.usedCount < coupon.usageLimit)))
  ...
  return <main className="mx-auto max-w-6xl px-4 py-16"><h1 className="text-4xl font-black">Current Offers</h1><div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{visible.map(({ offer, coupon }) => <article key={offer.id} className="overflow-hidden rounded-xl border bg-card"><>{offer.imageUrl && <img src={offer.imageUrl} alt="" className="h-44 w-full object-cover"/>}</><div className="p-5">{offer.badgeText && <p className="text-xs font-bold text-primary">{offer.badgeText}</p>}<h2 className="mt-1 text-xl font-bold">{offer.title}</h2><p className="mt-2 text-sm text-muted-foreground">{offer.shortDescription}</p>{coupon && <p className="mt-4 font-semibold">Code: {coupon.code} · {coupon.discountType === 'percent' ? `${coupon.discountValue}% off` : `$${coupon.discountValue} off`}</p>}<Link className="mt-5 inline-block font-semibold text-primary underline" href={`/offers/${offer.slug}`}>{offer.ctaLabel || 'View offer'}</Link></div></article>)}</div></main>
}
