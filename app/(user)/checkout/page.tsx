...
import { phoneInputCharacters } from '@/lib/validation/customer-input'
...
function checkoutAddress(saved: SavedAddress): Address {
  ...
  return { firstName: parts.shift() || '', lastName: parts.join(' '), address: saved.addressLine1, addressLine2: saved.addressLine2 || '', city: saved.city, state: saved.region || '', postalCode: saved.postalCode, country: saved.country, phone: phoneInputCharacters(saved.phone || '') }
}
...
export default function CheckoutPage() {
  ...
  const [settings, setSettings] = useState({ currency: 'AUD', taxEnabled: true, taxName: 'GST', taxRate: 10, shippingCost: 0, freeShippingThreshold: 50, bannerShippingBands: DEFAULT_BANNER_SHIPPING_BANDS })
  const [shippingProducts, setShippingProducts] = useState<Record<string, { freeShipping: boolean; customShippingAmount: string | null }>>({})
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => { try { const response = await fetch('/api/store/settings', { cache: 'no-store' }); if (!response.ok) throw new Error('Checkout settings are temporarily unavailable. Please reload to try again.'); setSettings((await response.json()).data); setSettingsReady(true) } catch (error) { setSettingsReady(false); setError(error instanceof Error ? error.message : 'Checkout settings could not be loaded.') } }
    window.addEventListener('focus', loadSettings)
    const loadProfile = async () => { const response = await fetch('/api/account/profile'); if (response.ok) { const profile = (await response.json()).data.profile; setEmail(profile.email || ''); setShipping((current) => ({ ...current, phone: phoneInputCharacters(profile.phone || current.phone) })) } }
  ...
  const productIds = [...new Set(items.map((item) => item.productId))].sort().join(',')
  useEffect(() => {
    let active = true
  ...
  const estimate = useMemo(() => { const discount = coupon ? coupon.discountCents / 100 : 0; const discountedSubtotal = Math.max(0, total - discount); const calculated = calculateShipping({ deliveryType, productSubtotal: discountedSubtotal, standardShippingCost: settings.shippingCost, freeShippingThreshold: settings.freeShippingThreshold, bannerBands: settings.bannerShippingBands, lines: items.map((item) => ({ quantity: item.quantity, width: Number(item.specifications?.width || 0), height: Number(item.specifications?.height || 0), unit: item.specifications?.unit || 'mm', productId: item.productId, freeShipping: shippingProducts[item.productId]?.freeShipping ?? item.specifications?.freeShipping === 'true', customShippingAmount: shippingProducts[item.productId]?.customShippingAmount != null ? Number(shippingProducts[item.productId].customShippingAmount) : null, isBanner: ['custom_banners','mesh_banners','vinyl_banners'].includes(item.specifications?.shippingCategory || '') })) }); const tax = checkoutTaxCents(Math.round(discountedSubtotal * 100), Math.round(calculated.amount * 100), settings.taxEnabled, settings.taxRate) / 100; return { discount, shipping: calculated.amount, tax, bannerAreaM2: calculated.bannerAreaM2, total: discountedSubtotal + calculated.amount + tax } }, [coupon, deliveryType, items, settings, shippingProducts, total])
  ...
  const fields = (which: 'shipping' | 'billing', value: Address) => <div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold">First name<Input required maxLength={100} className="mt-1" value={value.firstName} onChange={(event) => update(which, 'firstName', event.target.value)} /></label><label className="text-sm font-semibold">Last name<Input required maxLength={100} className="mt-1" value={value.lastName} onChange={(event) => update(which, 'lastName', event.target.value)} /></label><label className="text-sm font-semibold sm:col-span-2">Address line 1<Input required maxLength={500} className="mt-1" autoComplete="address-line1" value={value.address} onChange={(event) => update(which, 'address', event.target.value)} /></label><label className="text-sm font-semibold sm:col-span-2">Address line 2<Input maxLength={500} className="mt-1" autoComplete="address-line2" value={value.addressLine2} onChange={(event) => update(which, 'addressLine2', event.target.value)} /></label><label className="text-sm font-semibold">Suburb<Input required maxLength={160} className="mt-1" autoComplete="address-level2" value={value.city} onChange={(event) => update(which, 'city', event.target.value)} /></label><label className="text-sm font-semibold">State / territory<select required className="mt-1 h-10 w-full rounded-lg border bg-background px-3" value={value.state} onChange={(event) => update(which, 'state', event.target.value)}><option value="">Select state</option>{AUSTRALIAN_STATES.map((state) => <option key={state} value={state}>{state}</option>)}</select></label><label className="text-sm font-semibold">Postcode<Input required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} className="mt-1" autoComplete="postal-code" value={value.postalCode} onChange={(event) => update(which, 'postalCode', event.target.value.replace(/\D/g, '').slice(0, 4))} /></label><label className="text-sm font-semibold">Country<Input readOnly aria-readonly="true" className="mt-1" value="Australia" /></label><label className="text-sm font-semibold sm:col-span-2">Phone<Input required type="tel" inputMode="tel" pattern="(?:0[234578][0-9]{8}|\+61[234578][0-9]{8})" maxLength={12} autoComplete="tel" className="mt-1" value={value.phone} onChange={(event) => update(which, 'phone', phoneInputCharacters(event.target.value))} /><span className="mt-1 block text-xs font-normal text-muted-foreground">Use 04xxxxxxxx or +61xxxxxxxxx.</span></label></div>

  async function submit(event: React.FormEvent) {
    event.preventDefault(); if (submitting) return
    if (!settingsReady || !shippingReady) { setError('Please wait for current shipping and tax settings to load.'); return }
