...
interface Product { id: string; name: string; description: string; basePrice: string; templateId: string | null; sizeMode: string; allowCustomDimensions: boolean; freeShipping?:boolean;customShippingAmount?:string|null; category?: { name: string; category?:string }; template: CompatibleTemplate | null; templates: CompatibleTemplate[]; images: Array<{ id: string; url: string; alt: string | null; isPrimary: boolean }>; sizes: Size[] }
...
function ProductDetailContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const temporaryProduct = temporaryCustomQuoteProduct(id)
