...
export default function CustomerOrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-background px-4 py-8"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_340px]"><div className="space-y-5">
      ...
      <section className="rounded-xl border bg-card p-5"><h2 className="font-bold">Delivery</h2><p>Expected printing: {order.expectedPrintingAt ? new Date(order.expectedPrintingAt).toLocaleString() : 'Not scheduled'}</p><p>Expected delivery: {order.expectedDeliveryAt ? new Date(order.expectedDeliveryAt).toLocaleString() : 'Not scheduled'}</p><p>{order.courierName || 'Courier not assigned'} {order.trackingNumber && `· ${order.trackingNumber}`}</p>{order.customerNotes && <p className="mt-2 text-sm">{order.customerNotes}</p>}</section>
