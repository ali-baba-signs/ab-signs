...
export default function ProfilePage() {
  ...
  async function load() {
    const [profilePayload, addressPayload, reviewPayload, orderPayload] =
      await Promise.all([
        fetch("/api/account/profile").then((response) => response.json()),
        fetch("/api/account/addresses").then((response) => response.json()),
        fetch("/api/reviews?mine=true").then((response) => response.json()),
        fetch("/api/orders").then((response) => response.json()),
      ]);
    if (profilePayload.error) throw new Error(profilePayload.error.message);
    setProfile({
      name: profilePayload.data.profile.name || "",
      email: profilePayload.data.profile.email || "",
      company: profilePayload.data.profile.company || "",
      phone: profilePayload.data.profile.phone || "",
      alternatePhone: profilePayload.data.profile.alternatePhone || "",
      deliveryInstructions:
        profilePayload.data.profile.deliveryInstructions || "",
    });
  ...
  async function saveProfile() {
    ...
    setMessage(
      response.ok
        ? "Profile saved."
        : payload.error?.message || "Profile save failed.",
    );
  }
  async function saveAddress() {
    ...
    const payload = await response.json();
    if (!response.ok)
      return setMessage(payload.error?.message || "Address save failed.");
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-8">
      <div className="mx-auto max-w-5xl">
        ...
        <h1 className="mt-3 text-3xl font-black">My Profile</h1>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <Link
            href="/account/orders"
            className="flex items-center gap-3 rounded-xl border bg-card p-4 font-bold hover:border-primary"
          >
            <Package className="text-primary" /> Previous orders, current orders
            and tracking
          </Link>
          <Link
            href="/account/offers"
            className="flex items-center gap-3 rounded-xl border bg-card p-4 font-bold hover:border-primary"
          >
            <Star className="text-primary" /> Available Offers and Voucher
            history
          </Link>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <section className="rounded-xl border bg-card p-5">
            <h2 className="font-bold">Contact Profile</h2>
            <div className="mt-4 space-y-3">
              {(["name", "company", "phone", "alternatePhone"] as const).map(
                (key) => (
              ...
              <label className="block text-sm font-semibold">
                ...
                <span className="mt-1 block text-xs font-normal text-muted-foreground">
                  Email is owned by your secure sign-in account and cannot be
                  changed from the contact profile.
                </span>
              </label>
              <label className="block text-sm font-semibold">
          ...
          <section className="rounded-xl border bg-card p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {input("Label", "label")}
              {input("Full Name", "fullName", true)}
              {input("Phone", "phone")}
              {input("Alternate Phone", "alternatePhone")}
              {input("Address line 1", "addressLine1", true)}
              {input("Address line 2", "addressLine2")}
              {input("Suburb", "city", true)}
              <label className="text-sm font-semibold">
                State / territory
                <select
                  required
              ...
              <div className="flex gap-2 sm:col-span-2">
                <Button className="flex-1" onClick={() => void saveAddress()}>
                  {editingAddressId ? <Save /> : <Plus />} Save Address
                </Button>
                {editingAddressId && (
                  <Button
                    variant="outline"
            ...
            {addresses.map((item) => (
              <article key={item.id} className="rounded border p-3">
                <p className="font-semibold">
                  {item.label} {item.defaultShipping && "· Shipping"}{" "}
                  {item.defaultBilling && "· Billing"}
                </p>
                <p className="text-sm">
                  {item.fullName}, {item.addressLine1}, {item.city}{" "}
                  {item.region}, {item.postalCode}, Australia
                </p>
        ...
        <section className="mt-6 rounded-xl border bg-card p-5">
          <h2 className="font-bold">Orders & Receipts</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Current orders appear first; delivered, completed, and cancelled
            orders remain in your history.
          </p>
          <div className="mt-4 space-y-3">
            {orders.map((order) => (
                      ...
                      .map((item) => (
                        ...
                        >
                          <Button size="sm">
                            Write a Review ·{" "}
                            {item.product?.name ||
                              item.specifications?.productName ||
                              "Product"}
                          </Button>
                        </Link>
                      ))}
        ...
        <section id="my-reviews" className="mt-6 rounded-xl border bg-card p-5">
          ...
          {reviews.length ? (
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              You have not submitted a product review yet. Review forms appear
