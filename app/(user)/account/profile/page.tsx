  ...
  return <main className="min-h-screen bg-background px-4 py-8"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-xl border bg-card p-5"><h2 className="font-bold">{editingAddressId ? 'Edit saved address' : 'Add saved address'}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">
        {input('Label', 'label')}{input('Full name', 'fullName', true)}{input('Phone', 'phone')}{input('Alternate phone', 'alternatePhone')}{input('Address line 1', 'addressLine1', true)}{input('Address line 2', 'addressLine2')}{input('Suburb', 'city', true)}<label className="text-sm font-semibold">State / territory<select required className="mt-1 h-10 w-full rounded border bg-background px-2" value={address.region} onChange={(event) => setAddress({ ...address, region: event.target.value })}><option value="">Select state</option>{AUSTRALIAN_STATES.map((state) => <option key={state}>{state}</option>)}</select></label><label className="text-sm font-semibold">Postcode<Input required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} className="mt-1" value={address.postalCode} onChange={(event) => setAddress({ ...address, postalCode: event.target.value.replace(/\D/g, '').slice(0, 4) })}/></label><label className="text-sm font-semibold">Country<Input readOnly aria-readonly="true" className="mt-1" value="Australia" /></label>
