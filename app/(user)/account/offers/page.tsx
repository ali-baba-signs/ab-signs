...
import { couponCustomers, couponRedemptions, coupons, offers } from '@/lib/db/schema'
...
export default async function AccountOffersPage() {
  ...
  const now = new Date()
  const [offerRows, assignments, redemptions] = await Promise.all([
  ...
  const visible = offerRows.filter(({ offer, coupon }) => offer.enabled && (!coupon || coupon.visibility === 'public' || (coupon.visibility === 'customer_specific' && assigned.has(coupon.id))))
  ...
  const OfferCards = ({ rows, empty }: { rows: typeof active; empty: string }) => rows.length ? <div className="mt-4 grid gap-4 md:grid-cols-2">{rows.map(({ offer, coupon }) => <article key={offer.id} className="rounded-xl border bg-card p-5"><p className="text-xs font-bold text-primary">{offer.badgeText || 'OFFER'}</p><h2 className="mt-1 text-xl font-bold">{offer.title}</h2><p className="mt-2 text-sm text-muted-foreground">{offer.shortDescription}</p>{coupon && <div className="mt-4 flex items-center justify-between gap-3"><div><p className="font-black">{coupon.code}</p><p className="text-sm">{coupon.discountType === 'percent' ? `${coupon.discountValue}% off` : `$${coupon.discountValue} off`}{coupon.endsAt ? ` · Ends ${coupon.endsAt.toLocaleDateString()}` : ''}</p></div><CopyCouponButton code={coupon.code}/></div>}<Link href={`/offers/${offer.slug}`} className="mt-4 inline-block text-sm font-semibold text-primary underline">View terms</Link></article>)}</div> : <p className="mt-3 text-sm text-muted-foreground">{empty}</p>
