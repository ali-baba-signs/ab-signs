...
try {
 const result=await pool.query("update coupons set description = $2 where code = $1 returning id, code, description, discount_type, discount_value, ends_at, usage_limit, per_customer_usage_limit, minimum_subtotal, max_discount_amount",['DEVANTU99B','TEST ONLY — REMOVE AFTER TESTING'])
