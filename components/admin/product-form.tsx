...
interface StandaloneSizeRow { id?: string; label: string; width: string; height: string; unit: string; unitPrice: string; enabled: boolean; variantType: string; sizeGroup: string; sideMode: string }
...
const blankSize = (): StandaloneSizeRow => ({ label: '500 × 1000 mm', height: '500', width: '1000', unit: 'mm', unitPrice: '0', enabled: true, variantType: '', sizeGroup: '', sideMode: 'single' })
    ...
    void (async () => {
      try {
        ...
        if (productId) {
          ...
          if (product.sizeMode !== 'template_sizes') setStandaloneSizes(product.sizes.map((size) => ({ id: size.id, label: size.label, width: String(size.width || ''), height: String(size.height || ''), unit: size.unit, unitPrice: String(size.unitPrice), enabled: Boolean(size.enabled), variantType: size.variantType || '', sizeGroup: size.sizeGroup || '', sideMode: size.sideMode || 'single' })))
