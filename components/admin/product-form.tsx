...
import { ArrowDown, ArrowLeft, ArrowUp, Plus, RotateCcw, Save, Star, Trash2 } from 'lucide-react'
...
interface SizeRow { id?: string; label: string; width: string; height: string; unit: string; unitPrice: string; enabled: boolean; variantType: string; sizeGroup: string; sideMode: string; assembledHeightDescription: string; fitMode: 'contain' | 'cover' | 'stretch'; safeMargin: string; bleed: string; trimMarks: boolean; isDefault: boolean; frontTemplateId: string; backTemplateId: string }
...
export function ProductForm({ productId }: { productId?: string }) {
  ...
  async function selectCroppedImage(file: File) {
    if (images.length >= 12) return setError('A product can have up to 12 images.')
    const row: ImageRow = { clientId: crypto.randomUUID(), url: URL.createObjectURL(file), alt: form.name || file.name, isPrimary: images.length === 0, status: 'pending', progress: 0, file, fingerprint: `${file.name}:${file.size}:${file.lastModified}` }
    setError('')
    setImages((current) => [...current, row])
    await uploadImage(row.clientId, file)
  }
  ...
  <form onSubmit={submit} className="mx-auto max-w-6xl space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <Link href={adminPath('/products')} className="inline-flex items-center gap-2 text-sm font-semibold">
          <ArrowLeft className="h-4 w-4" /> Products
        </Link>
        <h1 className="mt-2 text-3xl font-black">{productId ? 'Edit product' : 'Add product'}</h1>
      </div>
      <Button type="submit" disabled={submitting || uploadsInProgress}>
        <Save /> {submitting ? 'Saving…' : uploadsInProgress ? 'Waiting for images…' : 'Save product'}
      </Button>
    </div>

    {error && <div role="alert" className="rounded-md border border-red-200 bg-red-50 p-3 text-red-700">{error}</div>}
    {success && <div role="status" className="rounded-md border border-green-200 bg-green-50 p-3 text-green-700">{success}</div>}
    ...
    <section className="rounded-xl border bg-card p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="text-sm font-semibold">
          ...
          <Input
            required
            placeholder="e.g. Feather Flag Banner"
            value={form.name}
        ...
        <label className="text-sm font-semibold">
          ...
          <select
            value={form.sizeMode}
            onChange={(event) => setForm({ ...form, sizeMode: event.target.value as ProductSizeMode, allowCustomDimensions: event.target.value === 'fixed_variants' ? false : form.allowCustomDimensions })}
            className="mt-2 h-10 w-full rounded-md border bg-background px-3"
          >
            {PRODUCT_SIZE_MODES.map((mode) => <option key={mode} value={mode}>{mode.replaceAll('_', ' ')}</option>)}
          </select>
          <span className="mt-1 block text-xs font-normal text-muted-foreground">Templates automatically inherit enabled product sizes.</span>
        </label>
        <label className="text-sm font-semibold">
          Base price
          <Input
    ...
    <section className="rounded-xl border bg-card p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Every image shows its upload state.</p>
        </div>
      </div>
      <div className="mt-4"><ImageCropUpload label="Product image" recommendedWidth={1000} recommendedHeight={1000} disabled={images.length >= 12} onCropped={(file) => selectCroppedImage(file)} /></div>
      {images.length === 0 ? (
        <p className="mt-6 rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">No images uploaded.</p>
      ) : (
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {images.map((image, index) => (
            <div key={image.clientId} className="overflow-hidden rounded-lg border">
              ...
              <div className="space-y-2 p-3">
                ...
                <Input
                  ...
                  value={image.alt}
                  onChange={(event) => setImages((current) => current.map((item, position) => position === index ? { ...item, alt: event.target.value } : item))}
                />
                <div className="flex gap-1">
                  <button type="button" title="Set featured" onClick={() => setImages((current) => current.map((item, position) => ({ ...item, isPrimary: position === index })))} className="rounded border p-2">
                    <Star className={`h-4 w-4 ${image.isPrimary ? 'fill-primary text-primary' : ''}`} />
                  </button>
                  <button type="button" title="Move earlier" onClick={() => moveImage(index, -1)} disabled={index === 0} className="rounded border p-2">
                    <ArrowUp className="h-4 w-4" />
                  </button>
                  <button type="button" title="Move later" onClick={() => moveImage(index, 1)} disabled={index === images.length - 1} className="rounded border p-2">
                    <ArrowDown className="h-4 w-4" />
                  </button>
                  {image.status === 'failed' && image.file && (
                    <button type="button" title="Retry" onClick={() => void uploadImage(image.clientId, image.file!)} className="rounded border p-2">
                      <RotateCcw className="h-4 w-4" />
                    </button>
                  )}
                  <button type="button" title="Remove" onClick={() => void removeImage(index)} className="ml-auto rounded border p-2 text-red-600">
    ...
    <section className="rounded-xl border bg-card p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">Production sizes</h2>
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" size="sm" variant="outline" onClick={addFlagSizes}>
            <Plus /> Standard flag sizes
          </Button>
        </div>
      </div>

      {form.sizeMode !== 'fixed_variants' && (
        <div className="mt-3 flex max-h-32 flex-wrap gap-1 overflow-auto rounded bg-secondary p-2">
          {BANNER_SIZE_PRESETS.map(([height, width]) => (
            <button key={`${height}-${width}`} type="button" className="rounded border bg-background px-2 py-1 text-xs" onClick={() => addBannerPreset(height, width)}>
              {height} × {width} mm
            </button>
          ))}
        </div>
      ...
      <div className="mt-4 space-y-3">
        {sizes.map((size, index) => (
          <div key={size.id || index} className="grid gap-2 rounded border p-3 md:grid-cols-4">
            <Input
            ...
            {form.sizeMode === 'fixed_variants' && (
              <>
                <select aria-label="Flag type" className="h-10 rounded border px-2" value={size.variantType} onChange={(event) => updateSize(index, { variantType: event.target.value })}>
                  {FLAG_TYPES.map((value) => <option key={value}>{value}</option>)}
                </select>
                <select aria-label="Size group" className="h-10 rounded border px-2" value={size.sizeGroup} onChange={(event) => updateSize(index, { sizeGroup: event.target.value })}>
                  {FLAG_SIZE_GROUPS.map((value) => <option key={value}>{value}</option>)}
                </select>
                <select aria-label="Print sides" className="h-10 rounded border px-2" value={size.sideMode} onChange={(event) => updateSize(index, { sideMode: event.target.value })}>
                  {SIDE_MODES.map((value) => <option key={value}>{value}-sided</option>)}
                </select>
              </>
            )}
            <Input
            ...
            />
            <select aria-label="Unit" className="h-10 rounded border px-2" value={size.unit} onChange={(event) => updateSize(index, { unit: event.target.value })}>
              {['mm', 'cm', 'in', 'ft', 'm'].map((unit) => <option key={unit}>{unit}</option>)}
            </select>
            <Input
              ...
              onChange={(event) => updateSize(index, { unitPrice: event.target.value })}
            />
            <select aria-label="Artwork fit" className="h-10 rounded border px-2" value={size.fitMode} onChange={(event) => updateSize(index, { fitMode: event.target.value as SizeRow['fitMode'] })}>
              {['contain', 'cover', 'stretch'].map((mode) => <option key={mode}>{mode}</option>)}
            </select>
            <Input
              ...
              min="0"
              step="0.001"
              placeholder="Safe margin"
              value={size.safeMargin}
              onChange={(event) => updateSize(index, { safeMargin: event.target.value })}
            />
            <Input
              aria-label="Bleed"
              type="number"
              ...
              placeholder="Bleed"
              value={size.bleed}
              onChange={(event) => updateSize(index, { bleed: event.target.value })}
            />
            {form.sizeMode === 'fixed_variants' && (
              <Input
                aria-label="Assembled height"
                placeholder="Assembled height (e.g. 2.8m)"
                value={size.assembledHeightDescription}
                onChange={(event) => updateSize(index, { assembledHeightDescription: event.target.value })}
              />
            )}
            <label className="text-xs font-semibold">Front template<select className="mt-1 h-10 w-full rounded border px-2" value={size.frontTemplateId} onChange={(event) => updateSize(index, { frontTemplateId: event.target.value })}><option value="">No assigned template</option>{templates.filter((template) => template.status === 'active' && template.conversionStatus === 'ready').map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>
            <label className="text-xs font-semibold">Back template<select className="mt-1 h-10 w-full rounded border px-2" value={size.backTemplateId} disabled={size.sideMode !== 'double'} onChange={(event) => updateSize(index, { backTemplateId: event.target.value })}><option value="">Use front / none</option>{templates.filter((template) => template.status === 'active' && template.conversionStatus === 'ready').map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>
            <label className="flex items-center gap-2 text-sm">
              <input type="radio" name="default-size" checked={size.isDefault} onChange={() => updateSize(index, { isDefault: true })} /> Default
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={size.trimMarks} onChange={(event) => updateSize(index, { trimMarks: event.target.checked })} /> Trim marks
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={size.enabled} onChange={(event) => updateSize(index, { enabled: event.target.checked })} /> Enabled
            </label>
            <Button
              type="button"
              variant="outline"
              ...
              onClick={() => setSizes((current) => current.filter((_, position) => position !== index))}
            >
              <Trash2 /> Remove
            </Button>
          </div>
      ...
      {form.sizeMode !== 'fixed_variants' && (
        <label className="mt-4 flex items-center gap-2 text-sm">
          <input type="checkbox" checked={form.allowCustomDimensions} onChange={(event) => setForm({ ...form, allowCustomDimensions: event.target.checked })} /> Permit customer custom dimensions
        </label>
      )}
    </section>

    <div className="flex justify-end">
      <Button type="submit" size="lg" disabled={submitting || uploadsInProgress}>
        <Save /> {submitting ? 'Saving…' : 'Save product'}
      </Button>
    </div>
  </form>
</main>
