...
import { ArrowDown, ArrowLeft, ArrowUp, ImagePlus, RotateCcw, Save, Star, Trash2 } from 'lucide-react'
...
interface ProductData { id: string; sku: string; name: string; description: string; basePrice: string; categoryId: string; templateId: string | null; featured: boolean; active: boolean; images: ApiImage[]; sizes: Array<{ id: string; label: string; width: string; height: string; unit: string; unitPrice: string; enabled: boolean }> }

export function ProductForm({ productId }: { productId?: string }) {
  const router = useRouter()
  const [categories, setCategories] = useState<Category[]>([])
  const [templates, setTemplates] = useState<Template[]>([])
  const [form, setForm] = useState({ sku: '', name: '', description: '', basePrice: '', categoryId: '', templateId: '', featured: false, active: true })
    ...
    void (async () => {
      try {
        ...
        if (productId) {
          ...
          setForm({ sku: product.sku, name: product.name, description: product.description || '', basePrice: product.basePrice, categoryId: product.categoryId, templateId: product.templateId || '', featured: Boolean(product.featured), active: product.active !== false })
          ...
          setTemplatePrices(product.templateId ? product.sizes.map((size) => ({ templateSizeId: size.id, label: size.label, dimensions: `${size.width} x ${size.height} ${size.unit}`, unitPrice: String(size.unitPrice), enabled: Boolean(size.enabled) })) : [])
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(productId ? `/api/admin/products/${productId}` : '/api/admin/products', {
        ...
        body: JSON.stringify({ ...form, templateId: form.templateId || null, images: images.map((image, order) => ({ id: image.id, assetId: image.assetId, key: image.key, url: image.url, alt: image.alt, isPrimary: image.isPrimary, order })), templatePrices }),
      })
      const payload = await response.json()
  ...
  return <main className="min-h-screen bg-background px-4 py-8"><form onSubmit={submit} className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-xl border bg-card p-6"><h2 className="text-xl font-bold">Product information</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Editable template<select value={form.templateId} onChange={(event) => selectTemplate(event.target.value)} className="mt-2 h-10 w-full rounded-md border bg-background px-3"><option value="">No template</option>{templates.filter((item) => item.status === 'active' && item.conversionStatus === 'ready').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
    ...
    <section className="rounded-xl border bg-card p-6"><h2 className="text-xl font-bold">Inherited template sizes and pricing</h2><p className="text-sm text-muted-foreground">Physical dimensions are controlled by the selected template. This product stores only its price and availability for each template size.</p>{!form.templateId ? <p className="mt-5 rounded border border-dashed p-5 text-sm text-muted-foreground">No editable template selected. A standard non-template option will use the base price.</p> : <div className="mt-5 space-y-3">{templatePrices.map((price, index) => <div key={price.templateSizeId} className="grid items-center gap-3 rounded border p-3 sm:grid-cols-[1fr_1fr_160px_auto]"><div className="font-semibold">{price.label}</div><div className="text-sm text-muted-foreground">{price.dimensions}</div><Input aria-label={`${price.label} price`} type="number" min="0" step="0.01" value={price.unitPrice} onChange={(event) => setTemplatePrices((current) => current.map((row, position) => position === index ? { ...row, unitPrice: event.target.value } : row))} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={price.enabled} onChange={(event) => setTemplatePrices((current) => current.map((row, position) => position === index ? { ...row, enabled: event.target.checked } : row))} /> Enabled</label></div>)}</div>}</section>
