    ...
    <section className="rounded-xl border bg-card p-6">
      ...
      <div className="mt-4 space-y-4">
        {sizes.map((size, index) => (
          <div key={size.id || index} className="grid items-end gap-3 rounded-lg border bg-card/60 p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6">
            <label className="text-xs font-gray-900">
              Variant label
              <Input
                placeholder="e.g. Small / 2m"
                value={size.label}
            ...
            {form.sizeMode === 'fixed_variants' && (
              <>
                <label className="text-xs font-gray-900/5">
                  Flag type
                  <select className="mt-1 h-10 w-full rounded border bg-background px-2 text-xs" value={size.variantType} onChange={(event) => updateSize(index, { variantType: event.target.value })}>
                    {FLAG_TYPES.map((value) => <option key={value} value={value}>{value}</option>)}
                  </select>
                </label>
                <label className="text-xs font-gray-900">
                  Size group
                  <select className="mt-1 h-10 w-full rounded border bg-background px-2 text-xs" value={size.sizeGroup} onChange={(event) => updateSize(index, { sizeGroup: event.target.value })}>
                    {FLAG_SIZE_GROUPS.map((value) => <option key={value} value={value}>{value}</option>)}
                  </select>
                </label>
              </>
            )}
            ...
            <label className="text-xs font-gray-900">
              Unit
              <select className="mt-1 h-10 w-full rounded border bg-background px-2 text-xs" value={size.unit} onChange={(event) => updateSize(index, { unit: event.target.value })}>
                {['mm', 'cm', 'in', 'ft', 'm'].map((unit) => <option key={unit} value={unit}>{unit}</option>)}
              </select>
            </label>
            ...
            <label className="text-xs font-gray-900">
              Artwork fit
              <select className="mt-1 h-10 w-full rounded border bg-background px-2 text-xs" value={size.fitMode} onChange={(event) => updateSize(index, { fitMode: event.target.value as SizeRow['fitMode'] })}>
                {['contain', 'cover', 'stretch'].map((mode) => <option key={mode} value={mode}>{mode}</option>)}
              </select>
            </label>
