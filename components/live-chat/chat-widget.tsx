...
interface Message {
  ...
  text: string;
  sender: "user" | "support";
  buttons?: { label: string; url?: string; action?: SupportIntent }[];
}
const policyLinks = [
...
export function ChatWidget() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        /* Retry on the next poll while the chat remains open. */
      }
    };
  ...
  return (
    <section
      ...
      className="fixed bottom-3 right-3 z-40 flex h-[min(640px,calc(100dvh-1.5rem))] w-[calc(100vw-1.5rem)] max-w-96 flex-col overflow-hidden rounded-lg border bg-background shadow-xl sm:bottom-6 sm:right-6"
    >
      <header className="flex items-center justify-between border-b bg-card p-4">
        <div>
          <h2 className="font-semibold">Alibaba Signs Support</h2>
          <p className="text-xs text-muted-foreground">
            Instant FAQs · Team replies during business hours
          </p>
        </div>
        <button
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <div role="log" aria-live="polite" className="space-y-3">
          {messages.map((message) => (
            <p
              key={message.id}
              className={`break-words rounded-lg p-3 text-sm whitespace-pre-wrap ${message.sender === "user" ? "ml-6 bg-primary text-white" : "mr-3 bg-secondary"}`}
            >
        ...
        {reply?.intent === "products" && (
          <Link className="mt-3 block text-sm underline" href="/products">
            Browse Products
          </Link>
        )}
        {reply?.requiresHuman && (
          <Link className="mt-3 block text-sm underline" href="/contact">
            {reply.intent === "custom_quote"
              ? "Submit quote details and artwork"
      ...
      >
        <label className="text-xs font-semibold" htmlFor="support-chat-input">
          {reply?.collectOrderId ? "Order ID" : "Your message"}
        </label>
        ...
        <p className="text-xs text-muted-foreground">
          FAQs need no sign-in. Private tracking and team requests require
          sign-in.
        </p>
      </form>
    </section>
