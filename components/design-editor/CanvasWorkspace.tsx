...
export function CanvasWorkspace({ canvasRef, workspaceRef, guides, zoom, productConfig, onToggleGuides, onFit }: {
  ...
  return <section className="relative flex min-w-0 flex-1 bg-zinc-100">
    <div className="absolute left-3 top-3 z-10 rounded-md border border-zinc-200 bg-white p-1 shadow-sm"><div className="flex gap-2"><Button size="sm" variant="ghost" onClick={onFit}><Maximize /> Fit</Button><Button size="sm" variant="ghost" onClick={onToggleGuides}>{guides ? <Eye /> : <EyeOff />} Guides</Button><span className="self-center px-2 text-xs text-zinc-500">{Math.round(zoom * 100)}%</span></div>{guides && <div aria-label="Design guide key" className="flex flex-wrap gap-x-3 gap-y-1 border-t px-2 py-1.5 text-[11px]"><span title="This is the bleed area. Extend artwork beyond this line." className="cursor-help whitespace-nowrap"><i className="mr-1 inline-block h-0 w-4 border-t border-dashed border-[#ed1b68] align-middle"/>Bleed</span><span title="This is the final cutting boundary." className="cursor-help whitespace-nowrap"><i className="mr-1 inline-block h-0 w-4 border-t border-black align-middle"/>Cut</span><span title="This is the safe area. Keep important content inside this line." className="cursor-help whitespace-nowrap"><i className="mr-1 inline-block h-0 w-4 border-t border-dashed border-green-500 align-middle"/>Safety</span></div>}</div>
    <div ref={workspaceRef} className="flex min-w-0 flex-1 items-center justify-center overflow-auto p-12 lg:p-20"><div className="relative shrink-0 bg-white shadow-[0_15px_45px_rgba(0,0,0,.2)]"><Rulers config={productConfig} zoom={zoom} /><canvas ref={canvasRef} /></div></div>
  </section>
}
