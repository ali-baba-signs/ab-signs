  ...
  return <section className="bg-zinc-50 py-16 lg:py-24"><div className="mx-auto max-w-[1440px] px-4 sm:px-8">
    ...
    <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3">
      {products.map((product) => <article key={product.id} className="group flex min-w-0 flex-col rounded-xl border border-zinc-200 bg-white p-2 shadow-sm sm:p-3">
        ...
        <div className="flex flex-1 flex-col p-1 sm:p-3"><h3 className="mt-2 text-base font-black sm:text-xl"><Link href={`/products/${product.id}`}>{product.name}</Link></h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-zinc-500">{product.description}</p><p className="mt-3 text-xs font-bold uppercase tracking-wide text-zinc-600">From ${Number(product.basePrice).toFixed(2)} · {product.soldQuantity.toLocaleString()} sold</p><div className="mt-auto flex flex-col gap-2 pt-5 xl:flex-row"><Link href={`/products/${product.id}`} className="inline-flex min-h-11 flex-1 items-center justify-center rounded-md bg-[#111] px-2 text-sm font-bold text-white hover:bg-[#ed1b68]">View product</Link><Link href={`/design?productId=${product.id}`} className="inline-flex min-h-11 items-center justify-center rounded-md border border-zinc-300 px-2 text-sm font-bold hover:border-[#ed1b68] hover:text-[#ed1b68]">Customise</Link></div></div>
      </article>)}
    </div>
  </div></section>
}
